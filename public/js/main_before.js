...
Cesium.Ion.defaultAccessToken =
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJqdGkiOiI0ZjhkZGFhNC1iM2E0LTRlM2ItYjE4OS02Yjc2Mzc5NzUxZTAiLCJpZCI6NzI4OTYsImlhdCI6MTYzNjQ0NzEyNX0.4CNGCZdr5hH6njIB2gFU-W8oDb4Pafk-pmG9tdWQ2QY";
    ...
    eventHandler.setInputAction(function (movement) {
      ...
      if (lClickEventFlag) {
        ...
        if (cartesian) {
          ...
          var cartographic = Cesium.Cartographic.fromCartesian(cartesian); //좌표로 변환
...
function addOSMBuilding(cFlag) {
  if (osmPrimitive != undefined) {
    ...
    if (cFlag) {
      //##실습3.osm 건물 추가 소스 추가
      osmPrimitive = viewer.scene.primitives.add(Cesium.createOsmBuildings());
    }
  }
}
    ...
    if (cFlag) {
      assetBuilding = viewer.scene.primitives.add(
        new Cesium.Cesium3DTileset({
          ...
          url: Cesium.IonResource.fromAssetId("1590582"),
        })
      );
    }
  }
...
function getClusterData() {
  if (clusterDataSor != undefined) {
    ...
    const dataSourcePromise = viewer.dataSources.add(Cesium.KmlDataSource.load("/data/facilities.kml", options));
